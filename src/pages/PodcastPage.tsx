...
export default function PodcastPage() {
  ...
  const fetchSummaries = async () => {
    ...
    const { data } = await supabase
      .from("summaries")
      .select("id, original_source, summary_text, type, created_at")
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-2">Convert your summaries into audio podcasts</p>
      </div>
      ...
      {selectedSummary ? (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          ...
          <Card className="glass-card-strong p-6">
            <h2 className="font-semibold font-display mb-1">{selectedSummary.original_source}</h2>
